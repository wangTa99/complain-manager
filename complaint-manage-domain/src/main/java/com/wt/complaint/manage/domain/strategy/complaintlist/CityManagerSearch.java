...
@Service(StrategyConstant.CITY_MANAGER_SEARCH)
@Slf4j
public class CityManagerSearch extends AbstractComplaintListSearch {
    ...
    protected ComplaintListSearchGoIn preHandler(ComplaintListSearchGoIn goIn) {
        log.info("before CityManagerSearch#preHandler, goIn:{}", RetailJsonUtil.toJson(goIn));
        transformSearchKey(goIn);
        ComplaintListSearchGoIn newGoIn = genNewSearchGoIn(goIn);
        log.info("CityManagerSearch#preHandler success, newGoIn:{}", RetailJsonUtil.toJson(newGoIn));
        return newGoIn;
    }
    ...
    public ComplaintListSearchGoIn genNewSearchGoIn(ComplaintListSearchGoIn goIn) {
        ...
        // 查看大区权限
        newGoIn.getAfterSaleWorkbenchPermissionGroup().setAfterSaleWorkbenchPermissionType(PermissionTypeEnum.LITTLE_ZONE.getCode());
        return newGoIn;
    }
}
