...
@Service(StrategyConstant.CENTER_STAGE_EXPERIENCE_SPECIALIST_SEARCH)
@Slf4j
public class CenterStageExperienceSpecialistSearch extends AbstractComplaintListSearch {
    ...
    protected ComplaintListSearchGoIn preHandler(ComplaintListSearchGoIn goIn) {
        log.info("before CenterStageExperienceSpecialistSearch#preHandler, goIn:{}", RetailJsonUtil.toJson(goIn));
        transformSearchKey(goIn);
        ComplaintListSearchGoIn newGoIn = genNewSearchGoIn(goIn);
        log.info("CenterStageExperienceSpecialistSearch#preHandler success, newGoIn:{}", RetailJsonUtil.toJson(newGoIn));
        return newGoIn;
    }
    ...
    public ComplaintListSearchGoIn genNewSearchGoIn(ComplaintListSearchGoIn goIn) {
        ...
        // 查看所有权�?
        newGoIn.getAfterSaleWorkbenchPermissionGroup().setAfterSaleWorkbenchPermissionType(PermissionTypeEnum.ALL.getCode());
        return newGoIn;
    }
}
